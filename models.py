...
class EgoAttention(BaseModule, Configurable):
    ...
    def forward(self, ego, others, mask=None):
        ...
        #print(ego)
        #print('egoview',ego.view(self.config["feature_size"]))
        ...
        #print(query_ego.shape)
        if mask is not None:
            #print('maskbefore',mask)
...
class EgoAttentionNetwork(BaseModule, Configurable):
    ...
    def forward_attention(self, x):
        ...
        #we send to the MLP layers the feature vector of our car,(ego), and of the rest(others)
        #print(ego)
        ego, others = self.ego_embedding(ego), self.others_embedding(others)
        #print("Others Shape")
        #print(others.shape)
        #print(others)
        #HERE IS WHERE if there is self attention then we will use the self attention network checking the other cars
        ...
        #We return redirecting to the (3rd step), the attention layer(If we have used SELF ATTENTION LAYER; the values of ego, att are done)
