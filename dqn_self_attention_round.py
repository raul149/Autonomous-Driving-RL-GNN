...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 19 02:07:46 2020
...
if __name__ == "__main__":
    ...
    parser.add_argument('--wandb-project-name', type=str, default="Roundabout",
...
env = RoundaboutEnv(env_config)
env.config['offscreen_rendering'] = False if TRAIN else False
...
q_network = EgoAttentionNetwork(model_config).to(device)
target_network = EgoAttentionNetwork(model_config).to(device)
target_network.load_state_dict(q_network.state_dict())
...
if TRAIN:
    ...
    for _ in range(10):
        while True:
            logits,att_matrix = q_network.forward(obs.reshape((1,)+obs.shape))
            action = torch.argmax(logits, dim=1).tolist()[0]
            # TRY NOT TO MODIFY: execute the game and log data.
            next_obs, reward, done, _ = env.step(action)
            env.render()
