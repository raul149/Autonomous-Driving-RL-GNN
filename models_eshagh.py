...
# -*- coding: utf-8 -*-
"""
Created on Mon Oct 19 02:12:05 2020

@author: aznarr1
"""

## originally from here: https://github.com/eleurent/rl-agents/blob/master/rl_agents/agents/common/models.py
...
class EgoAttention(BaseModule, Configurable):
    ...
    def forward(self, ego, others, mask=None, matrixself=None):
        ...
        #2.only value
        result = self.attention_combine(value.reshape((batch_size, self.config["feature_size"])))
        #3 - multiply

        #print('AttMatrix Start')
        #print(attention_matrix)
        return result, attention_matrix, matrixself
...
class SelfAttention(BaseModule, Configurable):
    ...
    def forward(self, ego, others, mask=None):
        ...
        if mask is not None:
            mask = mask.view((batch_size, 1, 1, n_entities)).repeat((1, self.config["heads"], 1, 1))
            #print(mask)
        value, attention_matrix = attentionself(query_all, key_all, value_all, mask,
                                            nn.Dropout(self.config["dropout_factor"]))
        #1.HALf-HALF, input and result value
        #result = (self.attention_combine(value.reshape((batch_size, n_entities, self.config["feature_size"]))) + input_all)/2
        #2.ONLY Value
        #result = self.attention_combine(value.reshape((batch_size, n_entities, self.config["feature_size"])))
        #3 Multiply.
        result = self.attention_combine(value.reshape((batch_size, n_entities, self.config["feature_size"]))) * input_all
...
class EgoAttentionNetwork(BaseModule, Configurable):
    ...
    def forward(self, x):
        #The forward function sends the 1*15*7 (car*features)
        x = torch.tensor(x, dtype=torch.float32)
        #Value send to the forward attention.
        ego_embedded_att, att_matrixtosend, selfatt_matrix = self.forward_attention(x)
        return self.output_layer(ego_embedded_att), att_matrixtosend, selfatt_matrix

    def split_input(self, x, mask=None):
        # Dims: batch, entities, features
        #Takes the first value of the second position of the vector (OUR AUTONOMOUS CAR)
        ego = x[:, 0:1, :]
        #T
        others = x[:, 1:, :]
        if mask is None:
    ...
    def split_input_self(self, x, mask=None):
        # Dims: batch, entities, features
        #Takes the first value of the second position of the vector (OUR AUTONOMOUS CAR)
        ego = x[:, 0:1, :]
        #T
    ...
    def forward_attention(self, x):
        ...
        if self.self_attention_layer:
            self_att, selfatt_matrix = self.self_attention_layer(ego, others, mask)
            ego, others, mask = self.split_input_self(self_att, mask=mask)
        return self.attention_layer(ego, others, mask, selfatt_matrix)
...
def attentionself(query, key, value, mask=None, dropout=None):
    """
        Compute a Scaled Dot Product Attention.
    :param query: size: batch, head, 1 (ego-entity), features
    :param key:  size: batch, head, entities, features
...
def attentionself2(query, key, value, mask=None, dropout=None):
    """
        Compute a Scaled Dot Product Attention.
    :param query: size: batch, head, 1 (ego-entity), features
    :param key:  size: batch, head, entities, features
